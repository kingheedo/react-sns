...
const PostImages = ({ images, header, postid }) => {
    ...
    if (images.length === 1) {
        return (
            ...
            && (
            <>
            <CardImg1 variant="top" alt={images[0].src} src={`${images[0].src.replace(/\/thumb\//, '/original/')}`} onClick={onZoom} />
    ...
    if (images.length === 2) {
        return (
            ...
            {header === '수정'
                && (
            <>
                    <CardImg2 id ={images[0].id} onClick = {handleDeleteImage} variant="top" alt={images[0].src} src={`${images[0].src.replace(/\/thumb\//, '/original/')}`} />
                    <CardImg2 id ={images[1].id} onClick = {handleDeleteImage} variant="top" alt={images[1].src} src={`${images[1].src.replace(/\/thumb\//, '/original/')}`} />
            ...
            {header === '메인이미지'
                && (
            <>
                <CardImg2 variant="top" alt={images[0].src} src={`${images[0].src.replace(/\/thumb\//, '/original/')}`} onClick={onZoom}/>
                <CardImg2 variant="top" alt={images[1].src} src={`${images[1].src.replace(/\/thumb\//, '/original/')}`} onClick={onZoom}/>
                   {showImagesZoom && <ImagesZoom images={images} onClose={onClose} />}
            </>
            )}
            </>
